...
void postexpr()
{
    ...
    if (next == LEFTSQBR)
    {
     ...
     else if (next == LEFTBR)
     {
         ...
         for (i=0; i<n; i++)
         {
             ...
             if (mdj > 0)
             {
                 if(! (cur == IDENT && identab[reprtab[repr+1]+2] == mdj) )
    ...
    else if (leftansttype !=LINT && leftansttype != LCHAR && leftansttype != LFLOAT)
        error(wrong_operand);
...
void condexpr()
{
    ...
    if (next == QUEST)
    {
        ...
        {
            r = tree[adif];
            tree[adif] = globtype == LFLOAT ? tree[adif-1] = WIDEN, TExprend : NOP;
            adif = r;
        }
        while (adif);
...
void statement()
{
    ...
    else if (cur == IDENT && next == COLON)
    {
        ...
        flagsemicol = 0;
        totree(TLabel);
        if ( (rr1 = reprtab[repr+1]) > 0 && (ttc = identab[rr1+3]) < 0)
        {
            ttc = -ttc;
            identab[rr1+3] = tc;
            while (ttc)
